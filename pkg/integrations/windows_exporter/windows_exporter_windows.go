...
func New(logger log.Logger, c *Config) (integrations.Integration, error) {
	...
	collectors, err := buildCollectors(enabledCollectorNames, logger)
	if err != nil {
		return nil, err
	}
	...
	return integrations.NewCollectorIntegration(c.Name(), integrations.WithCollectors(
		...
		collector.NewPrometheus(c.Timeout, collectors, logger),
	)), nil
}
...
func buildCollectors(enabled []string, logger log.Logger) (map[string]collector.Collector, error) {
